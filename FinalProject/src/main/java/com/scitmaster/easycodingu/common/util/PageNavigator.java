...
public class PageNavigator {
	//페이지 관련 정보 
		private int countPerPage;		//페이지당 글목록 수 - 한 페이지에 보여줄 글의 개수
		private int pagePerGroup;		//그룹당 페이지 수 - 화면에서 페이지를 몇 쪽까지 보여줄 것인가
		private int currentPage;		//현재 페이지 (최근 글이 1부터 시작) - 현재 화면에 노출된게 몇번째 페이지냐
		private int totalRecordsCount;	//전체 글 수 - DB상의 전체 글의 개수
		private int totalPageCount;		//전체 페이지 수 - 페이지당 글 목록수로 전체 글 수를 나누고 몫 + 1 한 값
		private int currentGroup;		//현재 그룹 (최근 그룹이 0부터 시작)  - 0:1~5, 1:6~10, ...
		private int startPageGroup;		//현재 그룹의 첫 페이지 - 0그룹:1, 1그룹:6, ...
		private int endPageGroup;		//현재 그룹의 마지막 페이지 - 0그룹:5, 1그룹:10, ...
		private int startRecord;		//전체 레코드 중 현재 페이지 첫 글의 위치 (0부터 시작) - DB에서 가져올 첫번째 레코드의 위치
